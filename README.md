...
- get [-tags invoices,isp] [-date 2025-01-13] [-output 2025_invoice.pdf] [-consult] id
Get the document infos from id
If the -tags option is set the document is set to the specific tags.
The same for -date.
The option -output will write the content of the document to the given file, it is the only way of viewing the actual document.
The option -consult will open the current document with the default os program or the one specified for mime type in table consult_executable.
